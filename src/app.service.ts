...
export class AppService {
  ...
  constructor(
    ...
    this.code = Math.floor(10000 + Math.random() * 99999);
  }
  ...
  async signup(user: User): Promise<any> {
    ...
    try {
      await this.userRepository.save(reqBody);
    } catch (e) {
      throw new BadRequestException(e, 'worong request');
      // return new HttpException(e, HttpStatus.INTERNAL_SERVER_ERROR);
    }
    try {
      await this.sendConfirmationEmail(reqBody);
    } catch (e) {
      throw new BadRequestException(e, "email don't sent");
    }
  }
  ...
  async varifyAccount(code: string): Promise<any> {
    try {
      ...
      await this.userRepository.update(
        { email: user.email },
        { isVarrified: true, authConfirmToken: undefined },
      );
